...
The HTML file for this analysis is created in RStudio using [R Markdown](rmarkdown.rstudio.com) and the knitr package for the R statistical programming language. The R commands to produce the analysis are embedded in the document. RStudio users will be able to reproduced this study by copying the two files described above as well as the US\_terror.Rmd file into the working directory and using the knitr package to produce an HTML output document from the .Rmd file. The GitHub repository containing the .Rmd file and the small US dataset used for the analysis is [here](github.com/dhimrich/x).
...
## Make a data frame just for plotting
## Set up a column of all the years
plot3.df <- data.frame(iyear = (1992:2014))
...
plot3.df$perp.cat[is.na(plot3.df$perp.cat)] <- "N/A"
plot3.df$Incidents[is.na(plot3.df$Incidents)] <- 0

yi1 <- ggplot(filter(plot3.df, perp.cat %in% 
...
yi2 <- ggplot(filter(plot3.df, perp.cat %in% 
